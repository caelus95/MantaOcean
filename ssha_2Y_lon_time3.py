...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 22 21:37:50 2021
...
ugos = data_a_2Y.ugos.values
vgos = data_a_2Y.vgos.values
...
for i in range(t):
    ...
    # Curl.append(tmp)
# Curl = np.array(Curl)
# np.save('/home/caelus/dock_1/Working_hub/DATA_dep/Kuroshio/ALL/task_adt/Curl_py',Curl)




# Curl = np.load('/home/caelus/dock_1/Working_hub/DATA_dep/Kuroshio/ALL/task_adt/Curl2.npy')
...
CURL = xr.Dataset(
    ...
    coords={
        ...
        "time": (['time'], data_a.time[12:-12]),
        # "reference_time": pd.Timestamp("2014-09-05"),
    },
)


data_ac = xr.merge([data_a_2Y,CURL])


# data_a_3Y = data_a_3Y.loc[dict(time=slice('1994-01','2018-12'))]

t12 = data_ac.time.values
lon12 = data_ac.longitude.values 
figdata12 = data_ac.curl.values
...
data_ac =data_ac.where( (data_ac.longitude<mask_mlon) | (data_ac.longitude>mask_Mlon) |\
              (data_ac.latitude<mask_mlat) | (data_ac.latitude>mask_Mlat),drop=False)

lat31 = data_ac.latitude
lon31 = data_ac.longitude

figdata32 = data_ac.ugos
figdata33 = data_ac.vgos
r_x, r_y, r_data1, r_data2 = r_vector4cube(lon31,lat31,figdata32,figdata33,[3,3])
lon_m32, lat_m32 = np.meshgrid(r_x,r_y)
figdata32 = r_data1
figdata33 = r_data2

plt.quiver(r_data1.values[12,:,:],r_data2[12,:,:])
# -----------
...
w_path_sig = '/home/caelus/dock_1/Working_hub/DATA_dep/Kuroshio/ALL/task_adt/vorticity4/'
n = 12
while n < 312:  
...
plt.pcolormesh(lon_m11, t_m11,figdata11*10**(2),
...
plt.title('b) 6M filtered (19$\degree$N)', fontproperties='',loc='left',pad=15,fontsize=28,fontweight='bold')
...
fig, ax = plt.subplots(figsize=(8,15),linewidth=1)
plt.pcolormesh(lon_m12, t_m12,figdata12*10**(7),
               cmap=plt.cm.get_cmap('bwr'),shading='gouraud')
# plt.colorbar()
plt.clim(-7.,7.)
plt.axvline(x=lon11[19],color='k',linewidth=3,linestyle='--',alpha=.6)
...
plt.yticks(ticks=ytick_location, labels=ytick_labels, rotation=0, fontsize=18, alpha=.7)
plt.ylabel('$\it{Years}$',fontsize=20,fontweight='light',alpha=.7)
plt.title('c) 2Y filtered (19$\degree$N)', fontproperties='',loc='left',pad=15,fontsize=28,fontweight='bold')
