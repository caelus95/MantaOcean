...
w_path1 = '/home/caelus/dock_1/Working_hub/DATA_dep/Kuroshio/'+var_name+'_18_30_112_180_M.nc'
...
tmp_dataset = xr.open_dataset(r_path+nc_list[0],decode_times=True)

dates.(tmp_dataset.time.values)

dates.datestr2num(str(tmp_dataset.time.values[0])[:7])
...
for i in nc_list[1:]:
    ...
    if int(str(tmp_data.time.values[0]).split('-')[1])
